...
const getUserProfile = async (req, res, next) => {
    ...
    try {
        ...
        const [news] = await connection.query(
            `SELECT n.*,COUNT(l.id) likes ,COUNT(u.id) dislikes FROM news n LEFT JOIN user_like_news l ON n.id=l.idNews LEFT JOIN user_unlike_news u ON n.id=u.idNews WHERE n.idUser = ? GROUP BY n.id `,
            [idUserAuth]
        );
